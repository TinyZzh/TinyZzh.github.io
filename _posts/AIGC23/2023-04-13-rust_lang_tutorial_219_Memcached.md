...
使用Rust语言Memcached需要先创建一个连接。可以使用`memcached::Client`结构体来创建一个连接：
...
使用`Client::set`方法可以将数据存储到Memcached中：
...
使用`Client::get`方法可以从Memcached中获取数据：
...
使用`Client::delete`方法可以从Memcached中删除数据：
...
使用`Client::replace`方法可以替换Memcached中的数据：
...
使用`Client::add`方法可以向Memcached中添加数据：
...
使用`Client::increment`方法可以将Memcached中的值自增：
...
使用`Client::decrement`方法可以将Memcached中的值自减：
