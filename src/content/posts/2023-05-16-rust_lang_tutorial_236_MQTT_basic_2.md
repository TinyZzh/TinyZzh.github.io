...
在实际应用中，我们通常需要同时处理多个MQTT客户端连接，可以通过创建多个Client实例来管理。rumqttc模块不提供内置连接池，但可以手动创建多个客户端连接。
...
fn main() {
    ...
    for i in 0..10 {
        let mqtt_options = MqttOptions::new(format!("test-client-{}", i), "localhost", 1883);
        let (mut client, _) = Client::new(mqtt_options, 10);
...
这个示例中，我们通过循环创建了10个MQTT客户端连接，每个客户端使用不同的ID连接到本地的MQTT服务器。
...
```rust
use std::thread;
use rumqttc::{Client, MqttOptions, QoS, Event, Incoming, Publish};
    ...
    for event in connection.iter() {
        match event.unwrap() {
            Event::Incoming(Incoming::Publish(publish)) => {
                ...
                println!("Received message: {} from topic: {}", publish.payload, publish.topic);
            },
            _ => {},
        }
    }
...
impl Storage {
    ...
    fn insert_message(&mut self, publish: &Publish) -> rusqlite::Result<()> {
        ...
        let qos_val = publish.qos as i32;
        stmt.execute(rusqlite::params![publish.topic, publish.payload, qos_val])?;
        Ok(())
    }
}
```
