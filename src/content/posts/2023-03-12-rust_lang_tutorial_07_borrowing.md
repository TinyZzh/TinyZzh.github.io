...
在上面的代码中，我们声明了一个可变的整数 `x` ，然后创建了一个指向 `x` 的可变引用 `y` 。我们使用 `*y` 来访问 `x` 的值，并将其修改为 `10` 。注意，在修改可变引用所指向的值时，我们需要使用 `*` 符号来解引用引用。最后，我们打印 `y` 的值。注意：当可变引用 `y` 仍然有效时，不能同时通过 `x` 来访问该值。
...
在上面的代码中，我们声明了一个可变整数 `x` ，然后在一个代码块中创建了一个指向 `x` 的不可变借用 `y` 。在该代码块结束后， `y` 的生命周期就结束了，我们不能再使用它。然后，我们在另一个代码块中创建了一个指向 `x` 的可变借用 `z` ，并将 `x` 的值修改为 `10` 。注意，当我们创建可变借用时，不能同时存在其他的借用，因为这会导致数据竞争。这是Rust的内存安全机制的一部分。
...
在上面的代码中，我们创建了一个可变整数 `x` ，然后在一个代码块中创建了一个指向 `x` 的不可变引用 `y` ，并在该代码块中打印。在该代码块结束后， `y` 的生命周期结束。然后我们在另一个代码块中创建了一个指向 `x` 的可变借用 `z` ，使用 `modify_value` 函数来修改 `z` 所指向的值，并打印结果。注意，不可变借用和可变借用不能同时存在，必须通过作用域来分隔。
