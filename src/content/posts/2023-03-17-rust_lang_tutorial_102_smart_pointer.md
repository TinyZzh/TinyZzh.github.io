...
fn main() {
    let animal_mutex = Arc::new(Mutex::new(Animal::new(String::from("Tom"), 2)));

    let animal_mutex1 = Arc::clone(&animal_mutex);
    let animal_mutex2 = Arc::clone(&animal_mutex);

    let handle1 = thread::spawn(move || {
        let mut animal_ref = animal_mutex1.lock().unwrap();
        animal_ref.age += 1;
        println!("Age: {}", animal_ref.age);
    });

    let handle2 = thread::spawn(move || {
        let mut animal_ref = animal_mutex2.lock().unwrap();
        animal_ref.age += 2;
        println!("Age: {}", animal_ref.age);
    });
