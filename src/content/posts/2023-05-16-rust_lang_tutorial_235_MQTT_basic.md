...
fn main() {
    ...
    // 注意：rumqttc 没有提供 get_retained() 方法。
    // 订阅者订阅主题时会自动收到该主题的保留消息，
    // 无需主动获取。
}
```

这个示例中，我们创建了一个MQTT客户端，连接到本地的MQTT服务器，然后发送了一条保留消息到`test/topic`主题。在调用`publish`方法时，我们指定了消息的QoS为AtLeastOnce，表示消息至少要被传输一次，但不保证只传输一次。第三个参数`true`表示该消息为保留消息。

订阅者订阅该主题时，MQTT服务器会自动推送最新的保留消息给订阅者，无需主动调用方法获取。
...
fn main() {
    let mut mqtt_options = MqttOptions::new("test-lwt", "localhost", 1883);
...
这个示例中，我们创建了一个MQTT客户端，连接到本地的MQTT服务器。然后我们使用`LastWill::new`方法创建了一个Last Will和Testament，指定了主题、QoS和消息内容。注意：Last Will必须在创建Client之前通过`mqtt_options.set_last_will()`方法设置，而不是在Client创建之后调用。
...
fn main() {
    let mut mqtt_options = MqttOptions::new("test-5", "localhost", 8883);

    let mut root_store = RootCertStore::empty();
    let cert_file = &mut BufReader::new(File::open(Path::new("ca.crt")).unwrap());
    root_store.add_pem_file(cert_file).unwrap();
    let config = ClientConfig::builder()
        .with_root_certificates(root_store)
        .with_no_client_auth();
    mqtt_options.set_tls(config);
...
其中，`ca.crt`是CA证书。TLS配置必须在创建Client之前通过`mqtt_options.set_tls()`方法设置，使用rustls的`ClientConfig::builder()`模式构建TLS配置。
